//constant and variable to call and create
...
//le bot received a message
client.on('message', handleMessage.bind(this));

//le bot received a speakinginput
...
//split the message content in syllabe
	var command = message.content.toLowerCase().slice(1).split(' ');
  if ((command[0] == 'play' && command[1] == 'list') || command[0] == 'playlist') { //if first syllabe is play and second is list or first is playlist then output command playlist
    command = 'playlist';
  }
  else {	//empty command
    command = command[0];
  }
// check command content and case split commands
	  switch (command) {
	    case 'leave':
      case 'disconnect':
	      commandLeave(); //bot leaves
	      break;
	    case 'play':
...
//this is message command

//define commandSkip
function help() {
    textChannel.send("leave: makes the bot leave the channel. play: play music. playlist: play playlist. skip/next: skip current song. pause: pause the music. resume volume listen: listen to ur voice. stop reset clear repeat: repeat after you. image");
  }


//this is bot speaking
...
//this is bot listening to voices
function handleSpeaking(member, speaking) {
  // Close the writeStream when a member stops speaking
  if (!speaking && member.voiceChannel) { //si je parle pas
    let stream = listenStreams.get(member.id); //crée une var ID & NULL
